...
def sample_neg(t,label_size,left_size,rel_size,d_tuple,next_random):
    ...
    if rand <0.5:
        ...
        c_next_random = random_int32(&c_next_random)
        sample = (t[0],t[1],c_next_random%left_size)
        while sample in d_tuple:
            c_next_random = random_int32(&c_next_random)
            sample = (t[0],t[1],c_next_random%left_size)
        samples.append(sample)
...
def norm(v):
    ...
    if norm >=1:
        cdivide(v,norm)
def train(B,R,C,tuples,d_tuple,next_random,it,lr = 0.01,Verbose=True):
    n1 = len(B)
    n2 = len(R)
    n3 = len(C)
    ...
    for _ in xrange(n_batches):
        for _ in xrange(batchsize):
            ...
            cnt +=1
            pos_vec = tuple_sum(B,R,C,t)
            pos_scr= sqr(pos_vec)
            next_random,negs = sample_neg(t,n1,n3,n2,d_tuple,next_random)
            #print t, negs
            for neg in negs:
                neg_vec = tuple_sum(B,R,C,neg)
                neg_scr= sqr(neg_vec)
