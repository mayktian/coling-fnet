...
def evaluate(prediction, ground_truth,tier,depth=2):
    """
    Evaluation matrix.
    ...
    for i in ground_truth:
        p = set([y for y in prediction[i] if len(tier.get_type_path(y))==depth])
        g = set([y for y in ground_truth [i] if len(tier.get_type_path(y))==depth])
	if p==g and len(p)==0:continue
        if p == g:
            same += 1
	count+=1
        same_count = len(p&g)
        ...
        micro_recall += len(g)
    print micro_precision,micro_recall,micro_n
