...
label_bin_hier  = Word2Vec.load_word2vec_format(in_dir + '/labels_hier.bin')
...
C =np.asarray([label_bin_hier[a.id2label[i]] for i in range(len(label_bin.vocab))],dtype=np.float32)
B = np.dot(C,B)
next_random = 1
normalize(A)
normalize(B)
